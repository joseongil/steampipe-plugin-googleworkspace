...
package googleworkspace // import "google.golang.org/api/admin/reports/v1"
...
func (c *ActivitiesListCall) doRequest(alt string) (*http.Response, error) {
	...
	reqHeaders.Set("x-goog-api-client", "gl-go/"+GoVersion()+" gdcl/20210812")
...
func (r *ActivitiesService) Watch(userKey string, applicationName string, channel *Channel) *ActivitiesWatchCall {
	c := &ActivitiesWatchCall{s: r.s, urlParams_: make(URLParams)}
	c.userKey = userKey
	c.applicationName = applicationName
	c.channel = channel
	return c
...
func (r *CustomerUsageReportsService) Get(date string) *CustomerUsageReportsGetCall {
	c := &CustomerUsageReportsGetCall{s: r.s, urlParams_: make(URLParams)}
	c.date = date
	return c
}
...
func (c *CustomerUsageReportsGetCall) doRequest(alt string) (*http.Response, error) {
	...
	// f2, _ := os.Create("/tmp/f4.txt")
	// fmt.Fprintf(f2, "%v", urls)
	// f3, _ := os.Create("/tmp/f5.txt")
	// fmt.Fprintf(f3, "%v\n%v", req, req.Header)
	return SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *CustomerUsageReportsGetCall) Do(opts ...googleapi.CallOption) (*UsageReports, error) {
	...
	// respb,_ := io.ReadAll(res.Body)
	// f2, _ := os.Create("./f4.txt")
	// fmt.Fprintf(f2, "%s", string(respb))
...
func (r *EntityUsageReportsService) Get(entityType string, entityKey string, date string) *EntityUsageReportsGetCall {
	c := &EntityUsageReportsGetCall{s: r.s, urlParams_: make(URLParams)}
...
func (r *UserUsageReportService) Get(userKey string, date string) *UserUsageReportGetCall {
	c := &UserUsageReportGetCall{s: r.s, urlParams_: make(URLParams)}
	c.userKey = userKey
	c.date = date
	return c
}
